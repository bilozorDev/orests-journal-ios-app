...
async def check_rate_limit(
    ...
    """
    window_start = (datetime.now(UTC) - RATE_LIMIT_WINDOW).replace(tzinfo=None)
...
async def handle_failed_invite_attempt(
    ...
    user.last_failed_invite_at = datetime.now(UTC).replace(tzinfo=None)

    # Check if we should trigger lockout
    if user.failed_invite_attempts >= LOCKOUT_THRESHOLD:
        user.is_locked_out = True
        user.lockout_expires_at = (datetime.now(UTC) + LOCKOUT_DURATION).replace(tzinfo=None)
