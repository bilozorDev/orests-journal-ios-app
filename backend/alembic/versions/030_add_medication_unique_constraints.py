...
def upgrade() -> None:
    ...
    # Keep one row for each medication_id + hour + minute combo using DISTINCT ON
    op.execute("""
        DELETE FROM medication_schedules
        WHERE id NOT IN (
            SELECT DISTINCT ON (medication_id, scheduled_hour, scheduled_minute) id
            FROM medication_schedules
            ORDER BY medication_id, scheduled_hour, scheduled_minute, created_at
        )
    """)
    ...
    # Keep one row for each medication_id + type + scheduled_time combo
    op.execute("""
        DELETE FROM notification_logs
        WHERE id NOT IN (
            SELECT DISTINCT ON (medication_id, notification_type, scheduled_time) id
            FROM notification_logs
            ORDER BY medication_id, notification_type, scheduled_time, sent_at
        )
    """)
